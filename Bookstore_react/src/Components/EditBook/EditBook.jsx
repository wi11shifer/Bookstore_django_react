...
import { Form, Input, Button, DatePicker, Select, message, Radio, Modal } from 'antd';
...
const EditBook = () => {
    ...
    axios.get(`http://localhost:8000/api/books/${book_id}/`)
      .then(res => {
        ...
        form.setFieldsValue({
          title: book.title,
          price: book.price,
          publication_date: book.publication_date ? moment(book.publication_date) : null,
          genre_id: book.genre?.genre_id || null,
          genre: {
            genre_name: book.genre?.genre_name || '',
          },
          author_id: book.author?.author_id || null,
          publisher_id: book.publisher?.publisher_id || null,
          author: {
            first_name: book.author?.first_name || '',
            last_name: book.author?.last_name || '',
            birthdate: book.author?.birthdate ? moment(book.author.birthdate) : null,
          },
          publisher: {
  ...
  const onFinish = async (values) => {
    ...
    formData.append('publication_date', values.publication_date ? values.publication_date.format('YYYY-MM-DD') : '');
    ...
    if (authorMode === 'select' && values.author_id) {
      ...
      formData.append('author.birthdate', values.author?.birthdate ? values.author.birthdate.format('YYYY-MM-DD') : '');
    }
    ...
    try {
      await axios.put(`http://localhost:8000/api/books/${book_id}/`, formData, {
        headers: {
  ...
  const handleDelete = () => {
    confirm({
      ...
      onOk: async () => {
        ...
        try {
          const response = await axios.delete(`http://localhost:8000/api/books/${book_id}/`);
          ...
          message.error(error.response?.data?.error || 'Failed to delete book');
        } finally {
      ...
      >
        ...
        <Form.Item label="Genre">
          ...
          >
            ...
            <Radio value="create">Update/Create genre</Radio>
          </Radio.Group>
        ...
        <Form.Item label="Author">
          ...
          >
            ...
            <Radio value="create">Update/Create author</Radio>
          </Radio.Group>
        ...
        <Form.Item label="Publisher">
          ...
          >
            ...
            <Radio value="create">Update/Create publisher</Radio>
          </Radio.Group>
        ...
        <Form.Item>
          ...
          <Button
            type="danger"
            class="dangerButton"
            style={{ marginLeft: '10px', backgroundColor: '#CA2B2B' }}
